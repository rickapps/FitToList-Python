...
GITHUB_URL = "https://github.com/rickapps/FitToList-Python"
...
class PhotoEditorApp(tk.Tk):
    ...
    def show_about(self):
        ...
        title_font.configure(weight="bold", size=title_font.cget("size") + 2)
        tk.Label(frame, text="FitToList", font=title_font).pack(anchor="w")
        tk.Label(
            frame,
            text="A desktop tool for quickly cropping and resizing photos in a folder.",
            justify="left",
            wraplength=320,
        ).pack(anchor="w", pady=(4, 8))
        tk.Label(
            frame,
            text="Designed by Rickapps. All code and documentation generated by Claude Code.",
            justify="left",
            wraplength=320,
        ).pack(anchor="w")

        link = tk.Label(frame, text=GITHUB_URL, fg=LINK_COLOR, cursor="hand2", justify="left")
        link_font = tkfont.nametofont(link.cget("font")).copy()
        link_font.configure(underline=True)
        link.configure(font=link_font)
        link.pack(anchor="w", pady=(8, 0))
        link.bind("<Button-1>", lambda e: webbrowser.open(GITHUB_URL))
        link.bind("<Enter>", lambda e: link.configure(fg=LINK_HOVER_COLOR))
        link.bind("<Leave>", lambda e: link.configure(fg=LINK_COLOR))

        tk.Button(frame, text="Close", command=dialog.destroy).pack(anchor="e", pady=(12, 0))
