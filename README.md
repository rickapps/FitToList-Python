...
*Designed by Rickapps. All code and documentation generated by Claude Code.*
...
- **Straighten** — draw a level line and drag either end (up to 30° in either direction) to trace a tilted feature like a horizon; releasing the drag levels the image immediately, and the line reappears flat so you can drag again to fine-tune without the image shrinking from repeated adjustments. Click elsewhere on the image, click Straighten again, or press Esc when you're done, and use Crop to Selection afterward to trim the corners it exposes — it never crops for you.
...
- **Straighten is a separate, mutually-exclusive overlay mode** (`self.straighten_active`/`self.straighten_angle`) layered onto the same canvas event handlers — while it's active they route to `_straighten_hit_test`/`_update_straighten_angle` instead of the crop-selection logic. It never rotates `current_image` (or its live thumbnail) while dragging, only a guide line drawn over the untouched image; releasing the drag calls `apply_straighten()`, which re-rotates a snapshot taken when the tool started (not the already-rotated `current_image`) by the cumulative angle so far, then resets the line to horizontal for another adjustment — this avoids compounding `expand=True`'s white-space padding and resample blur across repeated fine-tunes. Clicking off the line, toggling Straighten off, or Esc closes the tool.
